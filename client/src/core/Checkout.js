...
const Checkout = ({ products }) => {
  ...
  const showCheckout = () => {
    return isAuthenticated() ? (
      ...
      <Link to="/signin">
        <button className="w-full bg-gray-800 hover:bg-grey-900 text-white text-sm py-2 px-4 font-semibold rounded focus:outline-none focus:shadow-outline h-10">Sign in to checkout</button>
      </Link>
    );
  };
    ...
    <div onBlur={() => setData({ ...data, error: "" })}>
      {data.clientToken !== null && products.length > 0 ? (
        <div>
          ...
          >
            <label className="text-muted text-gray-500">Delivery address:</label>
            <textarea
  ...
  return (
    <div>
    <div className=" flex gap-2 mt-2">  <h2 className="text-muted text-gray-500">Total Amount:</h2> <div className="font-medium">₹ {getTotal()}</div></div>
